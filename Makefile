...
generate-manifests-sfcset: ## Generate manifests e.g. CRD, RBAC. for the sfcset controller.
	$(MAKE) clean-generated-yaml SRC_DIRS="./config/sfcset/crd/bases"
	$(CONTROLLER_GEN) \
	paths="./cmd/sfcset/..." \
	paths="./internal/sfcset/..." \
	paths="./api/sfc/..." \
	crd:crdVersions=v1 \
	rbac:roleName=manager-role \
	output:crd:dir=./config/sfcset/crd/bases \
	output:rbac:dir=./config/sfcset/rbac
	cd config/sfcset/manager && $(KUSTOMIZE) edit set image controller=$(SFCSET_IMAGE):$(TAG)
...
binary-sfcset: ## Build the sfcset controller binary.
	go build -trimpath -o $(LOCALBIN)/sfcset-manager gitlab-master.nvidia.com/doca-platform-foundation/dpf-operator/cmd/sfcset
