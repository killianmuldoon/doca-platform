...
var _ = Describe("DPUFlavor", func() {
	...
	Context("obj test context", func() {
		...
		It("create and get object minimal", func() {
			...
			data1, err := bfcfg.Generate(obj1, DefaultDPUName, "", false, "", string(provisioningv1.InstallViaHost))
...
`)
			...
			data2, err := bfcfg.Generate(obj2, DefaultDPUName, "", false, "", string(provisioningv1.InstallViaHost))
			Expect(err).To(Succeed())
...
`)
			...
			data, err := bfcfg.Generate(obj, DefaultDPUName, "", false, "", string(provisioningv1.InstallViaHost))
			Expect(err).To(Succeed())
