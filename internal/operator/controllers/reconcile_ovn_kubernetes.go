...
func (r *DPFOperatorConfigReconciler) reconcileCustomOVNKubernetesDeployment(ctx context.Context, dpfOperatorConfig *operatorv1.DPFOperatorConfig) error {
	...
	if err := deployCNIProvisioners(ctx, r.Client, dpfOperatorConfig); err != nil {
	...
	if err := deployCustomOVNKubernetes(ctx, r.Client, dpfOperatorConfig, r.Settings.CustomOVNKubernetesDPUImage, r.Settings.CustomOVNKubernetesNonDPUImage); err != nil {
...
func adjustCVO(ctx context.Context, c client.Client) error {
	...
	if err := c.Patch(ctx, clusterVersionCR, client.Apply, client.ForceOwnership, client.FieldOwner(dpfOperatorConfigControllerName)); err != nil {
...
func deployCNIProvisioners(ctx context.Context, c client.Client, dpfOperatorConfig *operatorv1.DPFOperatorConfig) error {
	dpuClusterClients, err := getDPUClusterClients(ctx, c)
	if err != nil {
		return fmt.Errorf("error getting clients for the DPU clusters: %w", err)
	}

	dpuCNIProvisionerObjects, err := deployDPUCNIProvisioner(ctx, c, dpuClusterClients, dpfOperatorConfig)
	if err != nil {
		return fmt.Errorf("error while deploying DPU CNI Provisioner: %w", err)
	}

	hostCNIProvisionerObjects, err := deployHostCNIProvisioner(ctx, c, dpfOperatorConfig)
	if err != nil {
...
func deployDPUCNIProvisioner(ctx context.Context, c client.Client, dpuClustersClients map[controlplane.DPFCluster]client.Client, dpfOperatorConfig *operatorv1.DPFOperatorConfig) ([]*unstructured.Unstructured, error) {
	var errs []error
...
func deployHostCNIProvisioner(ctx context.Context, c client.Client, dpfOperatorConfig *operatorv1.DPFOperatorConfig) ([]*unstructured.Unstructured, error) {
...
func deployCustomOVNKubernetes(ctx context.Context,
	...
	if err := deployCustomOVNKubernetesForWorkers(ctx, c, dpfOperatorConfig, customOVNKubernetesDPUImage); err != nil {
...
// allowed by design.
func deployCustomOVNKubernetesForWorkers(ctx context.Context, c client.Client, dpfOperatorConfig *operatorv1.DPFOperatorConfig, customOVNKubernetesDPUImage string) error {
	...
	if err := deployCustomOVNKubernetesEntrypointConfigMap(ctx, c, dpfOperatorConfig); err != nil {
		return fmt.Errorf("error while deploying custom OVN Kubernetes Entrypoint ConfigMap: %w", err)
	}

	if err := deployCustomOVNKubernetesDaemonSet(ctx, c, dpfOperatorConfig, customOVNKubernetesDPUImage); err != nil {
		return fmt.Errorf("error while deploying custom OVN Kubernetes ConfigMap: %w", err)
	}
	return nil
}
...
func deployCustomOVNKubernetesEntrypointConfigMap(ctx context.Context, c client.Client, dpfOperatorConfig *operatorv1.DPFOperatorConfig) error {
...
func deployCustomOVNKubernetesDaemonSet(ctx context.Context, c client.Client, dpfOperatorConfig *operatorv1.DPFOperatorConfig, customOVNKubernetesDPUImage string) error {
	ovnKubernetesDaemonset := &appsv1.DaemonSet{}
	key := client.ObjectKey{Namespace: ovnKubernetesNamespace, Name: ovnKubernetesDaemonsetName}
	if err := c.Get(ctx, key, ovnKubernetesDaemonset); err != nil {
		return fmt.Errorf("error while getting %s %s: %w", ovnKubernetesDaemonset.GetObjectKind().GroupVersionKind().String(), key.String(), err)
	}
	customOVNKubernetesDaemonset, err := generateCustomOVNKubernetesDaemonSet(ovnKubernetesDaemonset, dpfOperatorConfig, customOVNKubernetesDPUImage)
	if err != nil {
