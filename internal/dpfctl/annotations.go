...
// This file is an adoption from: https://github.com/kubernetes-sigs/cluster-api/blob/release-1.9/cmd/clusterctl/client/tree/annotations.go
...
const (
	...
	ShowObjectConditionsAnnotation = "tree.dpu.nvidia.com/show-conditions"
	...
	ObjectMetaNameAnnotation = "tree.dpu.nvidia.com/meta-name"

	// VirtualObjectAnnotation documents that the object does not correspond to any real object, but instead is
	// a virtual object introduced to provide a better representation of the DPF status, e.g. Applications.
	VirtualObjectAnnotation = "tree.dpu.nvidia.com/virtual-object"

	// GroupingObjectAnnotation is an annotation that should be applied to a node in order to trigger the grouping action
	// when adding the node's children. e.g. if you have a DPUSet, and you apply this annotation, then
	// the DPUs added as a children of this node will be grouped in case the ready condition
	// has the same Status and Reason.
	GroupingObjectAnnotation = "tree.dpu.nvidia.com/grouping-object"

	// GroupObjectAnnotation is an annotation that documents that a node is the result of a grouping operation, and
	// thus the node is representing group of sibling nodes, e.g. a group of DPUs.
	GroupObjectAnnotation = "tree.dpu.nvidia.com/group-object"

	// GroupItemsAnnotation contains the list of names for the objects included in a group object.
	GroupItemsAnnotation = "tree.dpu.nvidia.com/group-items"
	...
	ObjectZOrderAnnotation = "tree.dpu.nvidia.com/z-order"
)
...
// when adding the object's children. e.g. A DPUSet object, could be responsible of grouping
// the DPUs while added as a children objects.
...
// thus the object is representing group of sibling object, e.g. a group of DPUs.
...
// a virtual object introduced to provide a better representation of the DPF status.
...
func getBoolAnnotation(obj client.Object, annotation string) (bool, bool) {
	if val, ok := getAnnotation(obj, annotation); ok {
